...
field_to_index = 'ID'  # Field to index
...
mongo_collection.drop_index([(field_to_index, 1)])
print("MongoDB index dropped.")

# Benchmarking MySQL indexing speed with text index
print("Indexing MySQL...")
mysql_start_time = time.time()
with mysql_conn.cursor() as cursor:
    cursor.execute(f"ALTER TABLE updated ADD INDEX idx_{field_to_index} ({field_to_index})")
    mysql_conn.commit()
...
mongo_collection.create_index([(field_to_index, 1)])
...
plt.title(f'Indexing Speed Comparison for {field_to_index}')
plt.show()
