...
mysql_cursor.execute("SELECT data_length FROM information_schema.tables WHERE table_schema = 'books'")
mysql_size = mysql_cursor.fetchone()[0]

# Get the size of MongoDB collection
mongo_size = mongo_db.command("collstats", "updated")["storageSize"]
...
plt.xlabel('Database')
plt.ylabel('Size (GB)')
plt.title('Comparison of Database Sizes (Excluding Indexing)', fontsize=16)  # Set fontsize here
plt.show()
